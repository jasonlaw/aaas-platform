...
- **`scripts/setup-prerequisites.sh` — docker-group grant was silently
  skipped whenever Docker was already installed.** `sudo usermod -aG docker
  $USER` only ran inside the "Docker not yet installed" branch, so any host
  with Docker pre-installed (shared host, pre-baked image, Docker set up by
  something other than this script) never got the current user added to the
  `docker` group — and `DOCKER_GROUP_JUST_ADDED` stayed `false`, so the
  re-exec-under-new-group safety net a few steps later was skipped too, with
  no warning either way. Group membership is now checked and granted
  independently of the install branch, unconditionally and idempotently, on
  every run.

- **`platform/scripts/agent-vault-health.sh` — MITM proxy reachability check
  could never pass.** It compared `%{http_code}` against `407`/`200`, but
  curl's `--proxytunnel` mode treats a `407` response to `CONNECT` as a
  tunnel-establishment failure rather than an HTTP response, so `%{http_code}`
  stays `000` even when the proxy is up and correctly demanding auth. The
  check now parses the `CONNECT` status line out of verbose curl output
  instead.

- **11 SOP/skill/incident docs — bare invocations of
  `agent-vault-health.sh` produced false `FAIL`s on hosts where the
  operator account lacks Docker socket access**, surfacing as
  `agent_vault_container_not_running` and
  `agent_vault_health_check:unknown` even when Agent Vault is healthy.
  Invocations in `sop/monitor-health.md`, `sop/setup-agent-vault.md`,
  `sop/provision-tenant-vault.md`, `skills/manage-agent-vault.md`,
  `skills/handle-watchdog-alert.md`, `skills/setup-admin-hermes.md`,
  `skills/handle-tenant-request.md`, `incidents/agent-vault-failure.md`,
  `incidents/hermes-admin-failure.md`, and `PLATFORM-REFERENCE.md` now use
  `sudo -g docker` (not `sg` — see the 0.15.9-era note in
  `setup-prerequisites.sh` on `sg` being absent on minimal Ubuntu images).
  This is defense-in-depth on top of the `setup-prerequisites.sh` fix above,
  which addresses the underlying group-membership gap directly.
