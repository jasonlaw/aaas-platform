...
- **Agent Vault's MITM proxy must stay scoped to the LLM provider host.** `provision-tenant-vault` sets `NO_PROXY` for non-LLM hosts (Telegram, etc.) and sets each vault's unmatched-host policy to deny, so the proxy neither silently intercepts unrelated traffic nor passes unmatched requests through unmanaged.
