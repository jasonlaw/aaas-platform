...
- Manage Agent Vault (inspect, add/rotate credentials, mint/revoke tokens, configure tenant credentials via bidirectional channel): /opt/aaas/platform/skills/manage-agent-vault.md
...
- **LLM API key changes are not handled through the update-tenant SOP.** Because LLM keys live exclusively in Agent Vault (not in `.env`), changing one requires updating the vault entry directly — offboard and re-onboard the tenant, or contact the platform operator to update it in Agent Vault. In the future, when the bidirectional channel between the admin agent and tenant agents is implemented, the tenant agent will be able to signal a key change request; the admin agent will handle the vault update from there. Non-LLM credential changes (Telegram tokens, webhook secrets, etc.) go through the update-tenant SOP step 3 as normal `.env` edits.
