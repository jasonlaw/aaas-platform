...
- Tenant knowledge vault (durable, structured, owner-browsable notes - customers, suppliers, recurring patterns, reference material, and current operational facts at `Reference/Business Data.md`): /opt/aaas/tenants/{id}/vault/ - maintained by the tenant agent at runtime, built up gradually from conversation, never seeded into Mnemosyne
...
- Agent Vault health check: /opt/aaas/platform/scripts/agent-vault-health.sh (run as `sudo -g docker ...` — the `aaas` user has no direct Docker socket access; see docker-group note below)
...
- **`scripts/setup-platform.sh` uses one shared `MANAGED_ASSET_RELATIVE_PATHS` list for both the pre-flight repo check and the post-install `--validate-only` check.** When adding a new file to any managed directory (`sop/`, `skills/`, `scripts/`, `incidents/`, `policy/`, `checklists/`, `harness/`, `evals/`, `admin-hermes/`, `tenant-hermes/`), add it to `MANAGED_ASSET_RELATIVE_PATHS` once — never add a path only to one of the two validation functions.
...
- **Credential storage by type:** LLM API keys live exclusively in Agent Vault — the tenant's `.env` holds only the `routed-via-agent-vault` placeholder; the real key is never written to disk. All other credentials (Telegram tokens, webhook secrets, SMTP passwords, etc.) are non-LLM and live in `.env`. The tenant agent may append a single non-LLM `KEY=value` line to `/opt/data/.env` after explicit owner confirmation in the same conversation, immediately followed by `--force-recreate`. The admin agent may write non-LLM credentials to its own `.env` at `/opt/aaas/platform/admin/.env`, and also to a tenant's `.env` at `/opt/aaas/tenants/{tenant-id}/.env` when handling non-LLM secret updates via the update-tenant SOP (step 3) — always with operator confirmation before writing. Neither agent ever writes credentials to Mnemosyne, a skill file, a vault note, or any other location — this is rule `no_credential_persistence` in `platform-policy.yaml`.
  **`ADMIN_HERMES_API_KEY` in a tenant's `.env`** is the API server token the tenant agent uses to reach admin Hermes's bidirectional channel (`tenant-contact-admin.md`). It is not an LLM key and is correctly stored directly in `.env` — Agent Vault's MITM proxy only handles HTTP/HTTPS calls to LLM providers, so this credential is out of scope for Agent Vault by design. It is written by onboarding step 6.4 and is the only credential in a tenant's `.env` that does not route through Agent Vault.
- Run `sudo -g docker /opt/aaas/platform/scripts/agent-vault-health.sh` at the start of every health check and before any onboarding operation to confirm the vault is reachable. The `aaas` account is not a member of the `docker` group (root-equivalent access is intentionally not granted by default), so `docker` and `agent-vault-health.sh` commands fail with a misleading "not running"/"unknown" result unless `docker` group access is invoked explicitly — `sg` is unavailable on minimal Ubuntu images (see `scripts/setup-prerequisites.sh`), so `sudo -g docker` is the supported form.
...
- **iptables must be in legacy mode as a general host baseline.** Verify with `iptables --version` (must show `legacy`); if not, switch with `sudo update-alternatives --set iptables /usr/sbin/iptables-legacy && sudo update-alternatives --set ip6tables /usr/sbin/ip6tables-legacy && sudo systemctl restart docker`. **This does not fix custom Docker bridge networks (e.g. `agent-vault-net`) on Docker 29.x** — Docker manages its own nftables ruleset for bridges regardless of the iptables alternative, and only auto-adds FORWARD/CT/MASQUERADE rules for the default `docker0` bridge. Run `/opt/aaas/platform/scripts/fix-docker-nftables.sh --check` after any Docker or network change, and `--install` once per host to persist the fix across reboots and daemon restarts. See `docs/troubleshooting.md` → "Docker 29.x Custom Bridge Network Has No Internet (nftables gap)".
...
- Every tenant must have a scaffolded `vault/` directory created during onboarding (step 4.1); if it is missing during troubleshooting or an update, run `/opt/aaas/platform/tenant-hermes/scripts/vault-init-tenant.sh` to repair it - this is safe to re-run and never overwrites existing tenant vault notes
...
- **Two separate systems share "memory" or "vault" in their name; do not conflate them.** Agent Vault stores tenant credentials. Mnemosyne is each tenant's in-conversation recall, queried by the tenant agent at runtime. Each tenant's knowledge vault at `/opt/aaas/tenants/{id}/vault/` is that tenant's own durable, structured, Obsidian-browsable second brain (customers, suppliers, recurring patterns, reference material, and current operational facts at `Reference/Business Data.md`). The admin agent scaffolds a tenant's knowledge vault once during onboarding (`onboard-tenant.md` step 4.1) using `/opt/aaas/platform/tenant-hermes/scripts/vault-init-tenant.sh`; after that, the tenant agent itself reads and writes its own vault notes at runtime, gradually, from real conversation - the admin agent does not maintain tenant vault content and has no equivalent vault of its own. The admin agent has no filesystem access into a running tenant container beyond `docker exec`, and `/opt/aaas/platform/` is not mounted into any tenant. The tenant agent has no equivalent platform-authored skill file for its own vault; its search-before-writing habit is written directly into `SOUL.md.template` and the generated `vault/README.md` "For the assistant" section, because the tenant agent has no `platform/skills/`-style loader the way the admin agent does - it only ever reads `SOUL.md` and files it is explicitly told to check.
