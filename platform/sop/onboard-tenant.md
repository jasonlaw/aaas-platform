...
10. Install/activate the Mnemosyne Hermes plugin inside the tenant volume, then restart only this tenant:
...
11. Seed Mnemosyne with `memories/MEMORY.md` and `memories/USER.md`:
...
12. Add or update the tenant entry in `/opt/aaas/platform/tenants.yaml`.
13. Send the welcome message through the tenant's Telegram bot to every numeric ID in `TELEGRAM_ALLOWED_USERS`. This only succeeds for users who have already opened the bot and sent `/start`; report any Telegram `403 Forbidden` result as "user must start the bot first":
...
14. Report tenant ID, container status, Telegram bot link, Mnemosyne activation/seed status, welcome message delivery status per user ID, and registry update status.
