...
If the finding is low-risk (doc/comment wording, an obvious typo, formatting, a stale example — nothing that can change runtime behavior, decision logic, or output) and outside protected automation or runtime behavior, it's fine to fix it on the spot rather than defer it — but write the report regardless. The report is what gets a live, one-off fix backported into the versioned source the operator actually maintains; note in the report that the fix was already applied live, and include the exact file and change so the operator can replicate it upstream.
