...
MANAGED_ASSET_RELATIVE_PATHS=(
  ...
  "reference/llm-provider-catalog.md"
...
install_assets() {
  ...
  mkdir -p "$PLATFORM_ROOT/reference"
  ...
  copy_tree "$ASSET_ROOT/reference" "$PLATFORM_ROOT/reference"
...
validate_install() {
  ...
  # Derived from MANAGED_ASSET_RELATIVE_PATHS rather than a separate
  # hand-maintained list — this function previously kept its own `required`
  # array that had drifted from MANAGED_ASSET_RELATIVE_PATHS more than once
  # (missing entries went unnoticed until someone hit a runtime error on a
  # file that was never actually verified present). Only two kinds of path
  # are added on top of the shared array: CHANGELOG.md (repo-root, same
  # special-case as backup_managed_assets/validate_installed_matches_source
  # above — it isn't under platform/ so isn't a MANAGED_ASSET_RELATIVE_PATHS
  # entry), and the handful of files that are generated during setup itself
  # rather than copied from ASSET_ROOT (tenants.yaml, docker-compose.yaml,
  # reports/INDEX.jsonl) — those were never "managed assets" and must stay
  # listed explicitly here since there's no source-of-truth array for them.
  local required=("$PLATFORM_ROOT/CHANGELOG.md")
  local relative_path=""
  for relative_path in "${MANAGED_ASSET_RELATIVE_PATHS[@]}"; do
    required+=("$PLATFORM_ROOT/$relative_path")
  done
  required+=(
