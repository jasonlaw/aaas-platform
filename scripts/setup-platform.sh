...
MANAGED_ASSET_RELATIVE_PATHS=(
  ...
  "skills/research-tenant-business.md"
  "sop/build-image.md"
  ...
  "scripts/diagnose-tenant-logs.sh"
  "scripts/install-tenant-scripts.sh"
  "scripts/provision-tenant-vault.sh"
  "scripts/repair-tenant-ownership.sh"
  "scripts/run-business-research-subagent.py"
  "scripts/upgrade-tenant.sh"
  ...
  "tenant-hermes/scripts/reconcile-plugins.sh"
  "tenant-hermes/scripts/seed-mnemosyne.py"
  "tenant-hermes/scripts/seed-vault-context.py"
  "tenant-hermes/scripts/tenant-entrypoint.sh"
  "tenant-hermes/scripts/tenant-install.sh"
)
...
install_assets() {
  ...
  chmod +x "$PLATFORM_ROOT/scripts/check-admin-drift.sh"
  chmod +x "$PLATFORM_ROOT/scripts/add-tenant-compose-service.sh"
  chmod +x "$PLATFORM_ROOT/scripts/backfill-tenant-vault.sh"
  chmod +x "$PLATFORM_ROOT/scripts/diagnose-tenant-logs.sh"
  chmod +x "$PLATFORM_ROOT/scripts/install-tenant-scripts.sh"
  chmod +x "$PLATFORM_ROOT/scripts/provision-tenant-vault.sh"
  chmod +x "$PLATFORM_ROOT/scripts/repair-tenant-ownership.sh"
  chmod +x "$PLATFORM_ROOT/scripts/upgrade-tenant.sh"
  chmod +x "$PLATFORM_ROOT/tenant-hermes/scripts/reconcile-plugins.sh"
  chmod +x "$PLATFORM_ROOT/tenant-hermes/scripts/seed-mnemosyne.py"
  chmod +x "$PLATFORM_ROOT/tenant-hermes/scripts/seed-vault-context.py"
  ...
  chmod +x "$PLATFORM_ROOT/scripts/run-business-research-subagent.py"
...
build_image() {
  ...
  # The Dockerfile COPYs agent-vault-ca.pem into the image so Docker can install
  # it as a trusted CA. The file is generated by Agent Vault on first boot and
  # must be fetched before the image can be built (setup-agent-vault SOP step 3).
  # On a fresh install Agent Vault has not been started yet, so the build would
  # fail immediately with "not found". Catch it here with a clear message rather
  # than letting Docker emit the cryptic COPY checksum error.
  if [ ! -f "$PLATFORM_ROOT/docker/agent-vault-ca.pem" ]; then
    error "agent-vault-ca.pem not found in $PLATFORM_ROOT/docker/
  The Docker image cannot be built until the Agent Vault MITM CA has been fetched.
  Complete the Agent Vault setup first (setup-agent-vault SOP step 3):
    curl -o $PLATFORM_ROOT/docker/agent-vault-ca.pem http://localhost:14321/v1/mitm/ca.pem
  Then re-run with --build-image."
  fi
...
validate_install() {
  ...
  local required=(
    ...
    "$PLATFORM_ROOT/tenant-hermes/evals/_skill-verification-primitives-v1.yaml"
    ...
    "$PLATFORM_ROOT/scripts/check-admin-drift.sh"
    ...
    "$PLATFORM_ROOT/policy/platform-policy.yaml"
    "$PLATFORM_ROOT/scripts/aaas-watchdog.sh"
    "$PLATFORM_ROOT/scripts/generate-platform-eval.sh"
    "$PLATFORM_ROOT/scripts/validate-platform-rules.sh"
    "$PLATFORM_ROOT/scripts/run-business-research-subagent.py"
    "$PLATFORM_ROOT/incidents/hermes-admin-failure.md"
    "$PLATFORM_ROOT/skills/grill-me.md"
    "$PLATFORM_ROOT/skills/setup-admin-hermes.md"
    "$PLATFORM_ROOT/skills/handle-watchdog-alert.md"
    "$PLATFORM_ROOT/skills/handle-tenant-request.md"
    "$PLATFORM_ROOT/skills/manage-agent-vault.md"
    "$PLATFORM_ROOT/skills/research-tenant-business.md"
    "$PLATFORM_ROOT/tenant-hermes/scripts/seed-vault-context.py"
    ...
    "$PLATFORM_ROOT/sop/improve-sop.md"
    ...
    "$PLATFORM_ROOT/scripts/add-tenant-compose-service.sh"
    "$PLATFORM_ROOT/scripts/backfill-tenant-vault.sh"
    "$PLATFORM_ROOT/scripts/diagnose-tenant-logs.sh"
    "$PLATFORM_ROOT/scripts/install-tenant-scripts.sh"
    "$PLATFORM_ROOT/scripts/provision-tenant-vault.sh"
    "$PLATFORM_ROOT/scripts/repair-tenant-ownership.sh"
    "$PLATFORM_ROOT/scripts/upgrade-tenant.sh"
    "$PLATFORM_ROOT/incidents/agent-vault-failure.md"
    "$PLATFORM_ROOT/admin-hermes/aaas-admin-hermes.service"
    ...
    "$PLATFORM_ROOT/tenant-hermes/policy/tenant-policy.yaml.template"
    "$PLATFORM_ROOT/tenant-hermes/scripts/reconcile-plugins.sh"
    "$PLATFORM_ROOT/tenant-hermes/scripts/seed-mnemosyne.py"
    "$PLATFORM_ROOT/tenant-hermes/scripts/tenant-entrypoint.sh"
    "$PLATFORM_ROOT/tenant-hermes/scripts/tenant-install.sh"
